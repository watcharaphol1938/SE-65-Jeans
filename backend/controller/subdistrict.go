...
func GetSubDistrict(c *gin.Context) {
	var subdistrict entity.SubDistrict
	id := c.Param("id")
	if err := entity.DB().Raw("SELECT * FROM subdistricts WHERE id = ?", id).Scan(&subdistrict).Error; err != nil {
		   c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		   return
	}
	c.JSON(http.StatusOK, gin.H{"data": subdistrict})
...
func ListSubDistricts(c *gin.Context) {
	...
	if err := entity.DB().Raw("SELECT * FROM subdistricts").Scan(&subdistricts).Error; err != nil {
		   c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		   return
	}
...
func DeleteSubDistrict(c *gin.Context) {
	id := c.Param("id")
	if tx := entity.DB().Exec("DELETE FROM subdistricts WHERE id = ?", id); tx.RowsAffected == 0 {
