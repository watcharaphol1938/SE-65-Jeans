...
func GetBloodType(c *gin.Context) {
	var bloodtype entity.BloodType
	id := c.Param("id")
	if err := entity.DB().Raw("SELECT * FROM bloodtypes WHERE id = ?", id).Scan(&bloodtype).Error; err != nil {
		   c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		   return
	}
	c.JSON(http.StatusOK, gin.H{"data": bloodtype})
...
func ListBloodTypes(c *gin.Context) {
	...
	if err := entity.DB().Raw("SELECT * FROM bloodtypes").Scan(&bloodtypes).Error; err != nil {
		   c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		   return
	}
