...
func GetMaritalStatus(c *gin.Context) {
	var maritalstatus entity.MaritalStatus
	id := c.Param("id")
	if err := entity.DB().Raw("SELECT * FROM maritalstatuses WHERE id = ?", id).Scan(&maritalstatus).Error; err != nil {
		   c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		   return
	}
	c.JSON(http.StatusOK, gin.H{"data": maritalstatus})
...
func ListMaritalStatuses(c *gin.Context) {
	...
	if err := entity.DB().Raw("SELECT * FROM maritalstatuses").Scan(&maritalstatuses).Error; err != nil {
		   c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		   return
	}
...
func DeleteMaritalStautus(c *gin.Context) {
	id := c.Param("id")
	if tx := entity.DB().Exec("DELETE FROM maritalstatuses WHERE id = ?", id); tx.RowsAffected == 0 {
